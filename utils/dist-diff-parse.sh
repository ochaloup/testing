...
echoc() {
  if [ $IN_COLOR ]; then echo -e "\e[00;31m${1}\e[00m"; fi
}
...
get_filename() {
  ...
  # check whether the name is not jar with some filename  
...
action_rule_diff() {
  ...
  case $TYPE in
    ...
    ;;
    Java) 
       # Java file will be skipped now 
       echod "$RAW_FILENAME is java class file - skipping"
    ;;
    *text*) 
       # need to check
