 ...
 */
public class StatefulRemoteClient2 {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("To exit enter 'q', to print list of strings number-times 'print <number>', to add string to list on server 'add':");
		System.out.print("$ ");
		...
		while (!"q".equals(input) && !"quit".equals(input)) {
			...
			if(input.startsWith("print")) {
				...
				Integer numberTimes = 1;
				if(splittedString.length > 1) {
					try {
						numberTimes = Integer.parseInt(splittedString[1]);
					} catch (Exception e) {
						// nothing what we want to know about
					}
				}
				...
				for(int i=0; i<numberTimes; i++) {
					...
					int number = 1;
					System.out.println("Nodename where we are calling to: " + statefulBean.getNodeName());
